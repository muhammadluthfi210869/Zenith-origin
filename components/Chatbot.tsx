...
const Chatbot: React.FC<ChatbotProps> = ({ isOpen, setIsOpen, initialMessage }) => {
            ...
            setTimeout(async () => {
                try {
                    ...
                    if (response.ok) {
                        ...
                        const botText = assistantMessage?.content || "Terima kasih atas informasinya!";
    ...
    const handleSendMessage = async () => {
        ...
        try {
            console.log("🚀 Using: meta-llama/llama-3.1-8b-instruct:free");
            ...
            const assistantMessage = data.choices[0]?.message;
            const botText = assistantMessage?.content || "Maaf, saya tidak dapat memproses permintaan Anda saat ini.";
        ...
        <>
            {!isOpen && (
                <button
                    ...
                    className="ai-glow fixed bottom-6 right-6 z-50 p-4 bg-luxury-gold text-black rounded-full shadow-2xl hover:bg-white hover:scale-110 transition-all duration-300 group animate-bounce"
